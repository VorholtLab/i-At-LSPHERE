function solution = optimizeCbModelminNormCoup(model, osenseStr, minNorm, allowLoops, zeroNormApprox, parameters)
...
if (noLinearObjective==1 && ~isempty(minNorm)) || (noLinearObjective==0 && solution.stat==1 && ~isempty(minNorm))
    if strcmp(minNorm, 'one')
        ...
        LPproblem2.A = [LPproblem.A sparse(size(LPproblem.A,1),2*nRxns); % Compatible with coupling constraints ARP 2022
